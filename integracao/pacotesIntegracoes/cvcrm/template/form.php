        ...
        <?php foreach($formConfiguracoes as $i => $form): ?>
            <div class="tab-pane fade <?php if($i == 0): ?>show active<?php endif; ?>" id="nav-<?= $form ?>-<?= $integracao ?>" role="tabpanel" aria-labelledby="nav-<?= $form ?>-tab-<?= $integracao ?>">
                ...
                <?php if($form == 'geral'): ?>
                    ...
                    <div class="row">
                        ...
                        <div class="form-check form-switch mb-3" style="margin-left: 20px;">
                            <input class="form-check-input" type="checkbox" name="<?= $form ?>-status" value="true" <?php if($cvcrmConfiguracoes[$form]['status'] == true): ?>checked <?php endif ?> >
                            <label class="form-check-label">Status</label>
                        </div>

                        <div class="mb-3 col-4 form-floating">
                            <input list="cvcrm-<?= $form ?>-url-list" type="text" name="<?= $form ?>-url" class="form-control" value="<?= $cvcrmConfiguracoes[$form]['url'] ?>" placeholder="url">
                            <label>url</label>
                        </div>

                        <datalist id="cvcrm-geral-url-list">
                            <option value="https://cevalente.cvcrm.com.br/api/set/interesses/novointeresse">
                                https://cevalente.cvcrm.com.br/api/set/interesses/novointeresse
                            </option>
                        </datalist>

                        <div class="mb-3 col-4 form-floating">
                            <input type="text" list="geral-acao-list" name="<?= $form ?>-acao" class="form-control" value="<?= $cvcrmConfiguracoes[$form]['acao'] ?>" placeholder="acao">
                            <label>Ação</label>
                        </div>
                        <datalist id="geral-acao-list">
                            <option value="cadastrar_interessado_hotsite">
                                cadastrar_interessado_hotsite
                            </option>
                        </datalist>
                ...
                <div class="row">
                    ...
                    <div class="mb-3 col-4 form-floating">
                        <input type="text" name="<?= $form ?>-idEmpreendimento" class="form-control" value="<?= $cvcrmConfiguracoes[$form]['idEmpreendimento'] ?>" placeholder="idEmpreendimento">
                        <label>idEmpreendimento</label>
                    </div>
